...
DATA SEGMENT PARA 'DATA'
	...
	; Added for Difficulty Mode
	TEXT_MAIN_MENU_DIFFICULT DB 'DIFFICULT - D KEY','$' ; text with the DIFFICULT message
...
;       If it reaches this point, the ball is colliding with the left paddle	
	...
	; Remodify by Isaac Added AI Player
	...
	CHECK_RIGHT_PADDLE_MOVEMENT:
		...
		; Check if we are in single-player mode
		CMP CURRENT_SCENE, 01h        ; Check if the current scene is the game
		JNE CHECK_PLAYER_PADDLE_MOVEMENT ; If not single-player, skip automatic movement

		; Automatic movement for right paddle toward the ball
		MOV AX, BALL_Y                 ; Load ball's Y position
		MOV BX, PADDLE_RIGHT_Y         ; Load right paddle's Y position
		CMP AX, BX                     ; Compare ball position with paddle position
		JG MOVE_RIGHT_PADDLE_DOWN      ; If ball is below paddle, move paddle down
		JL MOVE_RIGHT_PADDLE_UP        ; If ball is above paddle, move paddle up
		JMP EXIT_PADDLE_MOVEMENT       ; If ball is at the same level, exit
	...
	CHECK_PLAYER_PADDLE_MOVEMENT:
		; Check for manual movement for the right paddle
...
;       Shows the DIFFICULT message
		...
		MOV AH,09h                       ;WRITE STRING TO STANDARD OUTPUT
		LEA DX,TEXT_MAIN_MENU_DIFFICULT      ;give DX a pointer 
		INT 21h                          ;print the string
...
;       Check whick key was pressed
			...
			; Modded by Andriano collaborated with Isaac
			CMP AL,'D'
			JE START_DIFFICULT
			CMP AL,'d'
			JE START_DIFFICULT
		...
		START_SINGLEPLAYER:
			...
			MOV GAME_ACTIVE,01h
			MOV BALL_VELOCITY_X, 5              ; Reset ball velocity in X direction
			RET
		...
		START_DIFFICULT:
			MOV CURRENT_SCENE,01h
			MOV GAME_ACTIVE,01h
			MOV BALL_VELOCITY_X, 9        ; Call speed boost procedure
			RET
